...
public class ParserTest {
	...
	public void testConstant() {
		RGBColor actual = myParser.makeExpression("1").evaluate(null);
		assertTrue(WHITE.equals(actual));
		actual = myParser.makeExpression("-1").evaluate(null);
		assertTrue(BLACK.equals(actual));
		actual = myParser.makeExpression("0.5").evaluate(null);
		assertTrue(GRAY.equals(actual));
		actual = myParser.makeExpression(".5").evaluate(null);
		assertTrue(GRAY.equals(actual));
		try {
			myParser.makeExpression("0.5 0.5").evaluate(null);
	...
	public void testBinaryOps() {
		...
		actual = myParser.makeExpression(
				"(plus (plus 0.01 0.09) (plus 0.4 0.5))").evaluate(null);
		assertTrue(WHITE.equals(actual));
		actual = myParser.makeExpression(
		...
		assertTrue(WHITE.equals(actual));
		try {
			myParser.makeExpression("(fooo 0.1 0.9)").evaluate(null);
			fail("Exception should have been thrown");
		} catch (ParserException e) {
